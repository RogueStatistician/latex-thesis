...
\newtheorem{sprop}{Proposizione}[prop]
\begin{document}
\onehalfspace
\frontmatter
...
\section{I primi quattro momenti della distribuzione Normale Asimmetrica multidimensionale}
...
\end{thm}  
\section{Le proprietà statistiche della distribuzione Normale Asimmetrica multidimensionale}
Per le proprietà di seguito descritte si fa riferimento ad una variabile Normale Asimmetrica $p$-dimensionale $Y\sim SN_p(\Omega,\alpha)$. Queste proprietà possono essere divise in quattro categorie:  quelle utili per creare campioni casuali (I), quelle relative alle distribuzioni marginali (II), quelle riguardanti le trasformazioni lineari (III e IV) e quelle che riguardano la forma quadratica (V). Tutte le proprietà descritte valgono anche nel caso in cui $Y\sim SN_p(\xi,\Omega,\alpha)$ ma necessitano di una notazione più complicata che viene quindi omessa.

\begin{prop}[Azzalini e Capitanio, 1999]
sia $\left(\begin{array}{l} X_0 \\ X \end{array}\right)\sim N_{p+1}(0,\Omega^*)$ con $X_0$ scalare e $\Omega^=\left[\begin{array}{ll} 1 & \delta^\top \\ \delta & \Omega \end{array}\right]$ matrice di correlazione, allora:
...
\end{array}\right.
$$
è una $SN_p(\Omega,\alpha)$ con:
$$
\alpha=\frac{\Omega^{-1}\delta}{(1-\delta^\top\Omega^{-1}\delta)^{1/2}}
$$
\end{prop}
Questa proposizione fornisce una rappresentazione stocastica di $Z$ che permette di generale valori pseudocasuali da una distribuzione Normale Asimmetrica $p$-dimensionale. La proposizione è facilmente estendibile ad una Normale Asimmetrica con vettore delle medie diverso dal vettore nullo applicando il teorema $\textbf{1.1}$.

\begin{prop}[Azzalini e Capitanio, 1999]
sia $Z\sim SN_p(\Omega,\alpha)$ e $Z$ sia divisa in $Z^\top=(Z_1^\top , Z_2^\top)$ di dimensioni rispettivamente $h$ e $p-h$, siano
$$
...
\end{array}\right)
$$
le corrispondenti partizioni di $\Omega$ e $\alpha$, allora la distribuzione marginale di $Z_1$ è ancora una $SN_h(\Omega_{11},\overline{\alpha}_1)$ con $overline{\alpha}_1=\frac{\alpha_1+\Omega_{11}^{-1}\Omega_{12}\alpha{2}}{(1+\alpha_2^\top\Omega_{22.1}\alpha_2)^{1/2}}$ e $\Omega_{22.1} = \Omega_{22}-\Omega_{21}\Omega_{11}^{-1}\Omega_{12}.$
\end{prop}
Quello che si ricava da questa proposizione è che sottoinsiemi di componenti di $Z$ hanno distribuzione marginale che è ancora una variabile Normale Asimmetrica, quindi ciascuna componente marginale di $Z$ si distribuisce come una variabile casuale Normale Asimmetrica univariata. 

\begin{prop}[Azzalini e Capitanio, 1999]
sia $Z\sim SN_p(\Omega,\alpha)$ e $A$ una matrice non singolare $p\times p$ tale che $A^\top\Omega A$ sia una matrice di correlazione, allora
$$
A^\top Z \sim SN_p(A^\top \Omega A,A^{-1}\alpha)
$$
\end{prop}

\begin{prop}[Azzalini e Capitanio, 1999]
sia $Z\sim SN_p(\Omega,\alpha)$, esiste una trasformazione lineare $Z^*=A^*Z$ tale che $Z^*\sim SN_p(I_p,\alpha^*)$, dove al più una sola componente di $\alpha$ è diversa da $0$.
\end{prop}

Questo risultato permette di definire una "forma canonica" per la distribuzione Normale Asimmetrica multivariata, le cui componenti sono mutualmente indipendenti con una sola componente a rappresentare tutta l'asimmetria della distribuzione multivariata. Se solo una delle componenti del vettore $\alpha$ è diversa da $0$, questo implica che non solo tutta l'asimmetria della distribuzione venga "assorbita" da una componente, ma anche la curtosi. Esisterà, perciò, al più una sola componente con curtosi diversa da quella di una Normale multivariata.
La densità di $Z^*$ è data da (Azzalini e Capitanio, 1999):
$$
2\prod_{i=1}^{p} \phi(u_i)\Phi(\alpha_m^*u_m)
$$

con $\alpha_m^*=(\alpha^\top\Omega\alpha)^{1/2}$ unica componente (se esiste) di $\alpha^*$ diversa da $0$.
\begin{prop}[Azzalini e Capitanio, 1999]
Sia $Z\sim SN_p(\Omega,\alpha)$ e $B$ matrice $p \times p$ simmetrica semi-definita positiva di rango $k$ tale che $B\Omega B=B$, allora $Z^\top B Z \sim \chi_k^2$

Generalizzando questa proposizione si ottiene 
\begin{sprop}[Azzalini e Capitanio, 1999]
Sia $Z\sim SN_p(\Omega,\alpha)$ e $C$ matrice a rango pieno $p\times k\,\,(p\leq k)$, allora $Z^\top C(C^\top \Omega C)^{-1} C^\top Z \sim \chi^2_p$
\end{sprop}
\end{prop}

Da queste due proprietà si ricava che $Z^\top\Omega Z\sim \chi^2_p$. 

Come si può notare questa proprietà non coinvolge il parametro $\alpha$
\thispagestyle{empty}
...
Per calcolare questo indice per una variabile casuale Normale Asimmetrica multidimensionale è sufficiente sostituire il momento non centrato di quarto ordine con quello della Normale Asimmetrica multidimensionale (paragrafo 1.1) dopo aver centrato la variabile $Y$. Si ottiene quindi:
$$
\begin{array}{lll}
...
$$
\end{defn*}
Utilizzando il momento quarto calcolato da Genton e coautori (2001) descritto nel paragrafo 1.1 è possibile ottenere l'indice per la distribuzione Normale Asimmetrica multidimensionale come:
...
Come nei casi precedenti per la distribuzione Normale Asimmetrica multidimensionale è sufficiente sostituire la matrice $M_4$ presentata nel paragrafo 1.1  calcolata da Genton e coautori  ottenendo quindi:
$$
\begin{array}{lll}
...
Definite le varie misure di curtosi è ora possibile procedere ad un confronto tra queste al variare di $\Omega$ ed $\alpha$. Saranno considerate qui di seguito sempre distribuzioni Normali Asimmetriche centrate ($\xi=\underline{0}$). Le tabelle di seguito riportare mostrano il valore degli indici di Mardia e Malkovich-Afifi al variare sia di $\Omega$ che di $\alpha$, mentre per gli altri indici il calcolo viene effettuato solo al variare della matrice di varianze e covarianze $\Omega$, questo perchè sono basati sul momento quarto non centrato di variabili casuali Normali Asimmetriche multidimensionali standardizzate che risulta indipendente dal valore di $\alpha$ come risulta da $1.3$.
...
\appendix
	...
	Di seguito vengono descritte alcune funzioni matematiche ed algebriche utilizzate per il calcolo degli indici.
		...
		\end{defn*}
		\section{Teorema per il calcolo dei momenti misti}
		Questo teorema è utile per il calcolo dei momenti misti per distribuzioni inserite in uno spazio sferico.
		\begin{defn*}[Fang,Kolz e Ng, 1990]
		sia $u^{(n)}$ distribuita uniformemente su una sfera con superficie in $\Re^{n}$ per qualsiasi intero $m_1,\ldots,m_n$ con
		$$
		m = \sum_{i=1}^{n} m_i
		$$	
		i momenti misti di $u_{(n)}$ possono essere espressi come
		$$
		E
		\left(
		\prod_{i=1}^{n} u_i^{m_i}
		...
		\left\{ 
		\begin{array}{ll}
		\frac{1}{(n/2)^{[l]}}\prod_{i=1}^{n} \frac{(2l_i)!}{4^{l-i}(l_i)!} & \text{se } m_i=2l_i \text{ è pari} \\
		0 & \text{se anche uno degli } m_i \text{ è dispari}
		\end{array}
		\right.
		$$
		con $x^{[l]}=x(x+1)\ldots(x+l-1)$.
		\end{defn*}
